...
public OpenAPI openAPI() {
    ...
    private void enrich(String entity, String rawPath, String method, Operation op) {
        if (op == null) return;
        if (alreadyDocumented(op)) return;
    ...
    /* ======================================================
     * UTIL – GROUP NAME & FORMAT
     * ====================================================== */
    private String groupName(String entity) {
        String title = toTitle(entity);
        return entity.contains("transaksi")
            ? "Transaksi • " + title
            : "Master • " + title;
    }
    ...
    enum EndpointDoc {
        ...
        String summary(String resource) {
            return switch (this) {
                ...
                case CREATE -> "Membuat " + resource + " baru (butuh token).";
                case BULK_CREATE -> "Membuat " + resource + " secara bulk (butuh token).";
                case UPDATE -> "Memperbarui " + resource + " (butuh token).";
                case DELETE -> "Menghapus " + resource + " (butuh token).";
                case BULK_DELETE -> "Menghapus " + resource + " secara bulk (butuh token).";
            };
        }

        String description(String resource) {
            return switch (this) {
                case GET_ALL -> "Mengambil seluruh data " + resource + ".";
                case GET_ONE -> "Mengambil detail " + resource + " berdasarkan ID.";
                case SEARCH -> "Mencari data " + resource + ".";
                case CREATE -> "Membuat data " + resource + " baru (butuh token).";
                case BULK_CREATE -> "Membuat banyak data " + resource + " dalam satu transaksi.";
                case UPDATE -> "Memperbarui data " + resource + " berdasarkan ID.";
                case DELETE -> "Menghapus data " + resource + " berdasarkan ID.";
                case BULK_DELETE -> "Menghapus banyak data " + resource + " sekaligus.";
            };
        }
    }
}
